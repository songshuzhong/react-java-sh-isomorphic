...
function table_colspan(table_id, table_rownum) {
    ...
    var table_currenttd = "";
    var table_SpanNum = 0;
    $(table_id + " tr:nth-child(" + table_rownum + ")").each(
            function(i) {
                ...
                table_row_obj.slice(table_mincolnum, table_maxcolnum).each(
                        function(i) {
                            if (i == 0) {
                                ...
                                var firstVal = $.trim(table_firsttd.text());
                                var currentVal = $.trim(table_currenttd.text());
                                if (firstVal != "" && firstVal == currentVal) {
                                    table_SpanNum++;
                                    if (table_currenttd.is(":visible")) {
                                        table_firsttd.width(parseInt(table_firsttd
                                                .width())
                                                + parseInt(table_currenttd.width()));
                                    }
...
function table_rowspan(table_id, table_colnum) {
    ...
    for (var j = 0; j < cols.length; j++) {
        $(table_id + " tr td:nth-child(" + cols[j] + ")").slice(table_minrow,
                table_maxrow).each(function(i) {
